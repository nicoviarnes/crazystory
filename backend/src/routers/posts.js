...
const selectPostStatement = `
  ...
  max(sr.name) category_name
  from posts p
  left join users u on p.author_id = u.id
  inner join categorys sr on p.category_id = sr.id
...
router.get('/', optionalAuth, async (req, res) => {
  try {
    const allowedFilters = ['category', 'author']
    const columnNamesEnum = {
      'category': 'max(sr.name)',
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const selectCategoryIdStatement = `select * from categorys where name = $1`

    const { rows: [foundCategory] } = await query(selectCategoryIdStatement, [category])
