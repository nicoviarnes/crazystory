...
const selectModeratorsStatement = `
  select u.username moderator_name, sr.name category_name
  from moderators m
  inner join users u on m.user_id = u.id
  inner join categorys sr on m.category_id = sr.id
`

const userIsModerator = async (username, category) => {
  const findCategoryModeratorStatement = `
    ${selectModeratorsStatement}
    where u.username = $1 and sr.name = $2
  `
  const { rows: [moderator] } = await query(findCategoryModeratorStatement, [
    username,
    category
  ])
