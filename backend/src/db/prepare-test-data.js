...
const prepareTestData = async () => {
  ...
  await query('delete from categorys')
  await query('alter sequence categorys_id_seq restart with 1')

  for (const category of categorys) {
    await query(`insert into categorys(name, description) values ($1, $2)`, [
      category.name,
  ...
  for (const post of posts) {
    const { rows: [foundCategory] } = await query(`select * from categorys where name = $1`, [
      post.category
    ])
    const insertPostStatement = `
      insert into posts(type, title, body, author_id, category_id)
      values($1, $2, $3, $4, $5)
      returning *
    `
    const author_id = userToIdMap[post.author]
    await query(insertPostStatement, [post.type, post.title, post.body, author_id, foundCategory.id])
  }
}
